...
function getNextFriday(date) {
  ...
  const nextFridayTimestamp = date.getTime() + daysUntilFriday * oneDay;
  const nextFriday = new Date(nextFridayTimestamp);
  return date.getDay() === 5
    ? new Date(nextFriday.getTime() + 7 * oneDay)
    : nextFriday;
}
...
function isDateInPeriod(date, period) {
  ...
  return dateToCheck >= start && end >= dateToCheck;
}
...
function formatDate(date) {
  ...
  let hours = String(dateObj.getUTCHours());
...
function getWeekNumberByDate(date) {
  ...
  let count = 0;
  for (let d = startOfYear; d <= targetDate; d.setDate(d.getDate() + 1)) {
...
function getNextFridayThe13th(date) {
  ...
  for (let month = startMonth; month < 12; month += 1) {
    const potentialDate = new Date(startYear, month, 13);
    if (potentialDate.getDay() === 5) {
      return potentialDate;
    }
  }

  for (let year = startYear + 1; year <= startYear + 1; year += 1) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      const formattedDate = `${`0${currentDate.getDate()}`.slice(-2)}-${`0${
